...
int in_cell_count2[8][2] = {0}; //count by unique location
//64km should be 0.53331 degree
double max_lon = min_lon + 0.53331 - 0.001;
double max_lat = min_lat + 0.53331 - 0.001;
//each cell is 250 m, 0.0020832 degree
...
//measure the beta cascade parameter
double beta_p[CELLNUM][CELLNUM][3][N] = {0};
int count_p[N][4]={0}; //N level, count 11,10,01,00
...
int trip_start_time[3][3][96] = { 0 }; //H,W,O: 0,1,2
int trip_start_duration[3][96][96] = {0}; // 2nd dimension is start time;
...
int main(){
	...
	while (getline(fid_in, tline)){
		...
		if (per_count>personal_limit&&per_count<10000 && place_count>2){
				...
				for (int i = 0; i<place_count; i++){
					if (place_lon[i]<min_lon || place_lon[i]>max_lon || place_lat[i]<min_lat || place_lat[i]>max_lat){
						...
						if (i == home){
							...
							for (int j=0;j<8;j++){
								xid1 = (int)((place_lon[i] - min_lon)/big_cell_sizes[j]);
								yid1 = (int)((place_lat[i] - min_lat)/big_cell_sizes[j]);
								
								xid2 = (int)((place_lon[home] - min_lon)/big_cell_sizes[j]);
								yid2 = (int)((place_lat[home] - min_lat)/big_cell_sizes[j]);
			...
			//if the trip is within each cell
			for (int i = 0; i<per_count-1; i++){
				if (lons[i]<min_lon || lons[i]>max_lon || lats[i]<min_lat || lats[i]>max_lat){
					continue;
				}
				
				if (lons[i+1]<min_lon || lons[i+1]>max_lon || lats[i+1]<min_lat || lats[i+1]>max_lat){
					continue;
				}
				
				if (locs[i] == home && locs[i+1] != home){
					for (int j=0;j<8;j++){
						xid1 = (int)((lons[i] - min_lon)/big_cell_sizes[j]);
						yid1 = (int)((lats[i] - min_lat)/big_cell_sizes[j]);
						
						xid2 = (int)((lons[i+1] - min_lon)/big_cell_sizes[j]);
						yid2 = (int)((lats[i+1] - min_lat)/big_cell_sizes[j]);
	...
	for (int i=0;i<8;i++){
		fprintf(fout_id,"%d %d\n",in_cell_count2[i][0],in_cell_count2[i][1]);
	}
